...
struct MRect
{
	...
	int32		width, height;
				...
				MRect(int32 inX, int32 inY, int32 inWidth, int32 inHeight);

				MRect(float inX, float inY, float inWidth, float inHeight);

	bool		Intersects(const MRect& inRHS) const;
	
	bool		ContainsPoint(int32 inX, int32 inY) const;
	
	void		PinPoint(int32& ioX, int32& ioY) const;

	void		InsetBy(int32 inDeltaX, int32 inDeltaY);
...
class MRegion
{
  public:
	...
	void		OffsetBy(int32 inX, int32 inY);
	bool		ContainsPoint(int32 inX, int32 inY) const;
...
template<class T>
inline
value_changer<T>::value_changer(T& inVariable, const T inTempValue)
...
inline
MRect::MRect(int32 inX, int32 inY, int32 inWidth, int32 inHeight)
	: x(inX)
	, y(inY)
	, width(inWidth)
	, height(inHeight)
...
inline
MRect::MRect(float inX, float inY, float inWidth, float inHeight)
...
inline bool MRect::ContainsPoint(int32 inX, int32 inY) const
{
	return
