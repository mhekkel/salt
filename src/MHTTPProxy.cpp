...
class proxy_connection : public enable_shared_from_this<proxy_connection>
{
  public:
	...
	static uint32 connection_count() { return s_connection_count; }
  ...
  private:
	...
	static uint32 s_connection_count;
};

uint32 proxy_connection::s_connection_count;
...
void MHTTPProxy::InitializeProxy(basic_connection* inConnection, uint16 inPort, bool require_authentication, log_level log)
{
	if (sInstance)
		sInstance->stop();
...
MHTTPProxy::MHTTPProxy(basic_connection* ssh_connection, bool require_authentication, uint32 log_flags)
...
void MHTTPProxy::set_log_flags(uint32 log_flags)
{
...
void MHTTPProxy::listen(uint16 port)
{
	if (m_log)
